...
// static constexpr int64_t requiredTicksForTimerInterval =
//     SysTickManager::calculateTicks(PARAM_TIMER_INTERVAL);

// static_assert(
//     requiredTicksForTimerInterval < SysTickManager::maxTicks && requiredTicksForTimerInterval >
//     0, "PARAM_TIMER_INTERVAL not possible with current clock config");
...
//     long long timeNow =
//         NOW(); // comment this out to improve performance, but: no time events any more
//     TimeEvent::propagate(
//         timeNow); // comment this out to improve performance, but: no time events any more
