...
HW_HAL_SPI::HW_HAL_SPI(
    SPI_IDX idx, GPIO_PIN sckPin, GPIO_PIN misoPin, GPIO_PIN mosiPin, GPIO_PIN nssPin)
    : m_idx { idx }
    , m_spi { spis[idx] }
    , m_misoPin { misoPin }
    , m_mosiPin { mosiPin }
...
bool HW_HAL_SPI::writeRead(
    const std::byte* txBuffer, size_t txSize, std::byte* rxBuffer, size_t rxSize)
{
    ...
    while (transmittedBytes < bytesToTransmit || receivedBytes < rxSize) {
        ...
        if (status.read<TNF>() && transmittedBytes < bytesToTransmit) {
            std::byte byteToWrite { transmittedBytes < txSize ? txBuffer[transmittedBytes]
                                                              : DUMMY_BYTE };
            transmittedBytes++;
