...
namespace dosis::support::crc {

/**
 * @brief A CRC checksum class using the Castagnoli CRC32 polynomial (0x1EDC6F41).
 * @note Used i.a. in the Cubesat Space Protocol (CSP).
 */
class CastagnoliCrc32 {
public:
    /**
     * @brief CRC lookup table for the Castagnoli CRC32 polynomial (0x1EDC6F41).
     */
    static constexpr uint32_t CRC_LOOKUP_TABLE[256] {
    ...
    /**
     * @brief Calculate CRC32 checksum for a given memory area.
     * @param addr Memory address of start byte of area to do checksum on.
     * @param length Length of memory area to do checksum on.
     * @return Calculated CRC32 checksum in host byte order.
     *
     * @warning Passing invalid pointer (nullptr) is undefined behavior!
     */
    static uint32_t calculateCRC32(const uint8_t* addr, uint32_t length)
    {
        uint32_t crc = CRC_INITIAL_VALUE;
        while (length-- != 0) {
            constexpr uint32_t BIT_MASK_8_BIT = 0xFF;
            // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
            crc = CRC_LOOKUP_TABLE[(crc ^ *addr++) & BIT_MASK_8_BIT] ^ (crc >> 8);
        }
