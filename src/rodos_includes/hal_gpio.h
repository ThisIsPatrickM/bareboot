...
typedef enum { // STM32F4		// AVR32 UC3A0512                   // NEXYS3-LEON3
    // Port A
    GPIO_000 =
        0, // PA00			// AVR32_PIN_PA00,                  // 7SEG_DISPLAY_4_A
...
enum class GPIO_OUTPUT_TYPE { PUSH_PULL = 0, OPEN_DRAIN };
...
public:
    ...
    int32_t init(
        bool isOutput = false,
        uint8_t numOfPins = 1,
        uint32_t initVal = 0x00,
        GPIO_OUTPUT_TYPE outputType = GPIO_OUTPUT_TYPE::PUSH_PULL);
    void reset();
    ...
    // dataReady == false
    // After this function, isDataReady() returns false, until the configured edge occurs on the
    // gpio pin. For e.g if you want to wait until a signal has high level, configure for rising
    // edge and enable interrupts. Then use this Function, then check if the signal already has high
    // level, if not do suspendUntilDataReady(), then the signal should have high level. This is all
    // thread save so you won't miss the edge while suspending
